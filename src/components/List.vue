<template>
    <v-container class="mb-8">
        <!-- <v-list class="mx-auto">
            ...
            >    
                <v-list-item-content>
                    <v-list-item-title class="nombre" v-text="item.name" @click="onNameClick"></v-list-item-title>
                </v-list-item-content>
                
                    
                <v-list-item-action>
                    <v-btn icon>
                        <v-icon color="grey lighten-1">fas fa-star</v-icon>
                    </v-btn>
                </v-list-item-action>
        ...
        <v-row v-for="(item ,i) in items" :key="i">
            <Item :item="item" @pokemon-name="pokemonNameListener" @pokemon-fav="pokemonFavListener"></Item>
        </v-row>
        
        <v-footer fixed>
            <v-layout justify-center class="mt-4">
                <v-btn class="mr-3 white--text" @click="onAllClick" :color="button_all_color" rounded width="150px"><v-icon class="mr-3">fas fa-list</v-icon>All</v-btn>
                <v-btn class="white--text" @click="onFavsClick" rounded width="150px" :color="button_favorites_color"><v-icon class="mr-3">fas fa-star</v-icon>Favorites</v-btn>
            </v-layout>
        </v-footer>         
        ...
        <v-dialog
            ...
            <v-card>
                <v-card-title class="world mb-3">
                    <v-btn class="close-dialog-icon" absolute top right icon fab @click="closeDialog"><v-icon>fa-times-circle</v-icon></v-btn>
                ...
                <v-list>
                    <v-list-item>
                        <span class="font-weight-bold">Name:</span><span class="pokemon-details-name ml-3">{{ pokemon.name }}</span>
                    </v-list-item>
                    <v-list-item>
                        <span class="font-weight-bold">Weight:</span><span class="ml-3">{{ pokemon.weight }}</span>
                    </v-list-item>
                    <v-list-item>
                        <span class="font-weight-bold">Height:</span><span class="ml-3">{{ pokemon.height }}</span>
                    </v-list-item>
                    <v-list-item>
                        <span class="font-weight-bold">Types:</span><span class="ml-3">{{ pokemon_types_str }}</span>
                    </v-list-item>
                </v-list>
                <v-layout justify-center>
                    <v-btn class="mr-3 white--text" rounded color="#f22539">Share to my friends</v-btn>
                    <v-btn icon><v-icon :color="dialog_star_color">fas fa-star</v-icon></v-btn>
                </v-layout>
                </v-card-text>
